...
for h in importance_vals:
    for p in prune_vals:
        ...
        test.fit(min_freq=10, stopwords=True)
        test.evaluate()

        num_of_edges.append(test.graph.number_of_edges())
        res_to_excel(test, "[CRAN]gsb_h_prune_num_edges.xlsx", dest_path, sheetname=f"GSB_{h}_{p}")
...
write(xl_namefile="[CRAN]gsb_h_prune_num_edges.xlsx", dest_path=dest_path, sheetname="GSB_comp_hyper_aggresive_v2", data=df)
