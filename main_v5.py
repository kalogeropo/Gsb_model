...
for i in range(0, 5):
    ...
    res_to_excel(N, "[NPL]GSBTesting.xlsx", dest_path, sheetname=name)
    print(mean(N.precision))
df = DataFrame(list(zip(list_to_total, test_name)), columns=["map", "Names"])
write(xl_namefile="[NPL]GSBTesting.xlsx", dest_path=dest_path, sheetname="windowsize_aggregate", data=df)
