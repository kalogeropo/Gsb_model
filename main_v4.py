...
for i in range(0,100,5):
    ...
    res_to_excel(N,"perc_windowTesting.xlsx",dest_path,sheetname=name)

df = DataFrame(list(zip(list_to_total, test_name)), columns=["map", "Names"])
write(xl_namefile="perc_windowTesting.xlsx", dest_path=dest_path, sheetname="perc_windowsize_aggregate", data=df)
