...
for i in wind_list:
    ...
    M = Gow(testcol, window = i, isdirected=False )
    M.fit()
    M.evaluate()
    name = f"GoW_{i}"
    list_to_total.append(mean(M.precision))
    test_name.append(name)
    res_to_excel(M, "[CRAN]test_GoW.xlsx", dest_path, sheetname=name)
df = DataFrame(list(zip(list_to_total, test_name)), columns=["map", "Names"])
write(xl_namefile="[CRAN]test_GoW.xlsx", dest_path=dest_path, sheetname="GoW_aggregate", data=df)
#NPL
